...
#This stores the serial and multithreaded execution times.
...
# This combines the current time with message and ensures to print controlled API requests. 

def log(message):
    line=f"{utc_now()} | {message}"
    with log_lock:
        # this adds the results folder. 
        LOG_FILE.parent.mkdir(parents=True,exist_ok=True)

        # This ensures log line is added to the end of the file and printed in the console.

        with LOG_FILE.open("a",encoding="utf-8") as f: f.write(line+"\n")
    print(line)

    # Wait for rate limit controls the requests and ensures not too many asre sent at the same time. 

def wait_for_rate_limit():
    while True:
        with rate_lock:

            # This checks the time of the first request in the deque and removes it if it is older than 60 seconds.

            now=time.monotonic()
            while request_times and now-request_times[0]>=60: request_times.popleft()

            #This ensures 100 requests per minute. If fewer than 100 requests are made then it allows new request.

            if len(request_times)<MAX_REQUESTS_PER_MINUTE:
                request_times.append(now); return
            
            #If the limit has been reached the program calculates how long it has to wait before allowing new requests. 
            wait=max(0.01,60-(now-request_times[0]))
        log(f"RATE_LIMIT wait_seconds={wait:.2f}"); time.sleep(wait)

        # Candle stick data which is returned by Binance is converted into named fields. 

def convert(symbol, record):

    """ Binance timestamps are measured in milliseconds.

Dividing by 1,000 converts milliseconds into seconds.

The value is then converted into a readable UTC datetime."""

    def ts(ms): return datetime.fromtimestamp(ms/1000,tz=timezone.utc).isoformat()

    """ Each position from the Binance record is assigned to a named column.

For example:

record[0] — open time.
record[1] — opening price.
record[2] — highest price.
record[3] — lowest price.
record[4] — closing price.
record[5] — volume."""
...
# This downloads the 1,000 records for one cryptocurrency.

def fetch_symbol(symbol):

# This creates the query parameters.
    ...
    with request_semaphore:
        ...
        try:
            ...
            # Converting Binance Json script to Python data

            with urlopen(f"{BASE_URL}?{params}",timeout=30) as response: payload=json.load(response)
        except Exception as exc:

            # The request start is written to the log.

            log(f"ERROR symbol={symbol} error={exc}"); raise # Raise sends the error upwards and cease request if error occurs.
        ...
        # Coverting the Binance Json script to Python data and storing it in rows.

        rows=[convert(symbol,r) for r in payload]; 

        # Successful request is written to the log with the number of records received and results recorded.
        
        log(f"END request symbol={symbol} records={len(rows)}"); return rows

def download_serial():  # Each symbol is downloaded on after the other. 
    rows=[]
    for symbol in SYMBOLS: rows.extend(fetch_symbol(symbol))
    return rows

def download_threaded(): # Stores each symbol's downloaded rows
    by_symbol={}

    # The pool containing  worker threads is created. 
